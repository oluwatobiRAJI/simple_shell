...
void help_env(void)
{
	char *msg = "env: env\n\tPrints the current envoirnment.\n";

	write(STDOUT_FILENO, msg, _strlen(msg));
}

/**
 * help_setenv - Displays infomation on the shell by builtin command 'setenv'
 */
void help_setenv(void)
{
	...
	write(STDOUT_FILENO, msg, _strlen(msg));
	msg = "enviroment variable, or modifies an existing one.\n\n";
	write(STDOUT_FILENO, msg, _strlen(msg));
...
/**
 * help_unsetenv - Displays information on the Shell by builtin command
 * 'unsetenv'
 */
...
int display_help(char **cmd, __attribute__((unused))int st)
{
	if (!cmd[1])
		...
		help_setenv();
	else if (_strlen(cmd[1], "unsetenv") == 0)
		help_unsetenv();
